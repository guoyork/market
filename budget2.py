...
if __name__ == "__main__":
    ...
    for t in range(10):
        for k in range(5):
            ...
            res2 = []
            res3 = []
            print("episode " + str(t)+" "+str(k) + " start")

            print("--------------------------")
            m = n * 20
            if os.path.exists(exp_name+"/ground_truth_"+str(t)+str(k)+".txt"):
                res0=np.loadtxt(exp_name+"/ground_truth_"+str(t)+str(k)+".txt").tolist()
                res2 = np.loadtxt(exp_name+""+"/opt_"+str(t)+str(k)+".txt").tolist()
            for s in range(len(res0),sample_size):
                ...
                for i in range(m//5*k):
                    match0[i]=match1[i]
                costs = np.random.random((m, n))

                w0 = np.concatenate((2 * np.random.lognormal(sigma=1/4, size=(m // 2, 1)), 4 * np.random.lognormal(sigma=1/4, size=((m + 1) // 2, 1))),
                                    axis=0)
                w1 = np.concatenate((np.random.lognormal(sigma=1/4, size=(m // 2, 1)), 2 * np.random.lognormal(sigma=1/4, size=((m + 1) // 2, 1))),
                                    axis=0)
                outcomes = np.maximum(vec2mat(match1, w1),vec2mat(match0, w0))
                budget = set_budget(match1, match0, costs)*(1+t/10)
                ...
                np.savetxt(exp_name+"/ground_truth_"+str(t)+str(k)+".txt", res0)
                np.savetxt(exp_name+"/opt_"+str(t)+str(k)+".txt", res2)
